...
export default function ExportPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <TopBar
        title="claude-code-analytics · export / import"
        subtitle="export your data or merge from another machine"
      />
          ...
          <div className="space-y-4">
            <div>
              <p className="text-[13px] text-muted-foreground mb-3">
                Exports stats, sessions, facets, and history as a portable{" "}
                <code className="text-foreground/80">.ccboard.json</code> file.
                You can import this on another machine to merge session data.
              </p>
              <p className="text-[12px] text-yellow-500/80 mb-3">
        ...
        <Card title="Import / Merge from Another Machine">
          <div className="space-y-4">
            <p className="text-[13px] text-muted-foreground">
              Import a <code className="text-foreground/80">.ccboard.json</code>{" "}
              file exported from another machine. This is an{" "}
            ...
            >
              <p className="text-[13px] text-muted-foreground">
                Drag <code>.ccboard.json</code> here or{" "}
              ...
              <div className="border border-border rounded p-4 space-y-2 text-[13px]">
                ...
                {importDiff.new_sessions === 0 ? (
                  <p className="text-muted-foreground/50 text-[12px] pt-1">
                    All sessions from this export are already present. Nothing
                    to merge.
                  </p>
                ) : (
                  <div className="pt-2 border-t border-border/50">
                    <p className="text-[12px] text-amber-400">
                      ⚠ Actual merge writing to ~/.claude/ is not implemented in
                      this version. This preview shows what would be merged.
                    </p>
                    <div className="mt-2 max-h-32 overflow-auto space-y-1">
                      {importDiff.sessions_to_add.slice(0, 10).map((s) => (
                        <div
                          key={s.session_id}
                          className="text-[12px] text-muted-foreground/60 font-mono"
                        >
