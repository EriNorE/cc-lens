...
export async function GET() {
  const [sessions, slugDirs] = await Promise.all([
    getSessions(),
    listProjectSlugs(),
  ]);
  ...
  for (const [projectPath, sessionList] of byPath.entries()) {
    ...
    const totalDuration = sessionList.reduce(
      (s, m) => s + (m.duration_minutes ?? 0),
      0,
    );
    const totalLinesAdded = sessionList.reduce(
      (s, m) => s + (m.lines_added ?? 0),
      0,
    );
    const totalLinesRemoved = sessionList.reduce(
      (s, m) => s + (m.lines_removed ?? 0),
      0,
    );
    const totalFilesModified = sessionList.reduce(
      (s, m) => s + (m.files_modified ?? 0),
      0,
    );
    const gitCommits = sessionList.reduce(
      (s, m) => s + (m.git_commits ?? 0),
      0,
    );
    const gitPushes = sessionList.reduce((s, m) => s + (m.git_pushes ?? 0), 0);
    const inputTokens = sessionList.reduce(
      (s, m) => s + (m.input_tokens ?? 0),
      0,
    );
    const outputTokens = sessionList.reduce(
    ...
    const estimatedCost = sessionList.reduce((sum, s) => {
      return (
        sum +
        estimateCostFromUsage(s.model ?? "claude-opus-4-6", {
