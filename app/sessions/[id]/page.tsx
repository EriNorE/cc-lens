...
import {
  UserTurnCard,
  AssistantTurnCard,
} from "@/components/sessions/replay/turn-cards";
...
export default function SessionDetailPage({
  ...
  const { data: metaData } = useSWR<{
    session: SessionMeta & { estimated_cost: number };
      ...
      <div className="flex flex-col min-h-screen">
        ...
        <div className="p-6 space-y-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className={`h-${i % 2 === 0 ? "16" : "24"} bg-muted rounded animate-pulse`}
            />
          ))}
        </div>
      </div>
  ...
  const projectName = meta
    ? projectDisplayName(meta.project_path ?? "")
    : id.slice(0, 8);
  ...
  const compactionByTurnIndex = new Map(
    replay.compactions.map((c) => [c.turn_index, c]),
  );
        ...
        {replay.compactions.length > 0 && (
          <>
            ...
            <span className="text-amber-400">
              ⚡ {replay.compactions.length} compaction
              {replay.compactions.length !== 1 ? "s" : ""}
            </span>
          </>
        )}
      </div>
